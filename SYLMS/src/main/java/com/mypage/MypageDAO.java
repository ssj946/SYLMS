...
public class MypageDAO {
	...
	public MypageDTO readMember(String id) {
       ...
       try {
		...
		sb.append(" a.id, a.name, a.pwd, a.tel, a.email, TO_CHAR(a.birth, 'yyyy-mm-dd') birth, d.departmentName  ");
		sb.append(" FROM account a ");
		sb.append(" LEFT JOIN professor p ");
		sb.append(" ON a.id = p.id ");
		sb.append(" LEFT JOIN subject s ");
	...
	public void updateMember(MypageDTO dto) throws SQLException {
		...
		try {
			sql = "UPDATE account SET pwd = ?, tel=?, email=?   WHERE id = ?";
